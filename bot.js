...
const slashCommands = [
 ...
 sub
 .setName('tarjeta')
 .setDescription('Publica en el chat la tarjeta interactiva navegable ( ) visible para todos')
 )
 .addSubcommand(sub =>
 sub
 .setName('listar')
...
function getPotionInfo(potionLevel) {
 ...
 if (lvl <= 60) {
 return { badge: ` [Lv. ${lvl}]`, color: '#57F287', name: `Poción Nivel ${lvl}` };
 }
 if (lvl <= 100) {
 return { badge: ` [Lv. ${lvl}]`, color: '#5865F2', name: `Poción Nivel ${lvl}` };
 }
 if (lvl <= 160) {
 return { badge: ` [Lv. ${lvl}]`, color: '#FEE75C', name: `Poción Nivel ${lvl}` };
 }
 return { badge: ` [Lv. ${lvl}]`, color: '#EB459E', name: `Poción Nivel ${lvl}` };
}
...
function buildQueueEmbed(queueData, pageInfo = null) {
 ...
 .setTitle(`${potionPrefix}${title || 'Cola de Turnos'} ${isClosed ? '[CERRADA]' : ''}`)
 .setColor(isClosed ? '#ED4245' : potion.color);

 let desc = description || '';
 if (isClosed) {
 desc = `*[CERRADA] Cola cerrada temporalmente (Abre a las 18:00 hrs Chile o con /cola abrir).* \n${desc}`.trim();
...
function buildCardViewerComponents(activeQueues, currentIndex) {
 ...
 .setLabel(` Ver anteriores colas (Pág. ${currentChunkPage}/${totalChunkPages})`)
 ...
 selectOptions.push(
 new StringSelectMenuOptionBuilder()
 .setLabel(` Ver más colas (Pág. ${currentChunkPage + 2}/${totalChunkPages})`)
